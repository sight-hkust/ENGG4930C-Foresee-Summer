    ...
    componentDidMount() {
        ...
        ref.child("records").on("value", (snapshot) => {
            ...
            this.setState({
                ...
                firstIndexToShow: Math.max(
                    0,
                    tempDate.length - UpperDisplayLimit
                ),
            });
        });
                            ...
                            <View style={RecordScreenStyle.contentContainer}>
                                <View style={RecordScreenStyle.eyesButton}>
                                    <TouchableOpacity
                                        activeOpacity={0.8}
                                        onPress={() =>
                                            this.setState({ Leye: true })
                                        }
                                    >
                                        <Image
                                            source={
                                                this.state.Leye ? Open : Close
                                            }
                                            style={{
                                    ...
                                    <TouchableOpacity
                                        activeOpacity={0.8}
                                        onPress={() =>
                                            this.setState({ Leye: false })
                                        }
                                        style={{ paddingLeft: 40 }}
                                    >
                                        <Image
                                            source={
                                                this.state.Leye ? Close : Open
                                            }
                                            style={{
                                                height: ScreenHeight / 10,
                                                resizeMode: "contain",
    ...
    return (
        ...
        >
            <Button
                icon={
                    <Icon
                        name={glassType ? "eyeglass" : "dehaze"}
                        type={glassType ? "simple-line-icon" : ""}
                        size={22}
                        color="#2D9CDB"
                    />
                }
                ...
                TouchableComponent={TouchableOpacity}
            />
            <Text style={{ color: "white", fontSize: ScreenHeight / 40 }}>
                {glassType ? "調整度數" : "真實度數"}
            </Text>
            <RenderModal
                data={data}
...
export const RenderModal = (props) => {
    ...
    return (
        ...
        >
            ...
            />
            <Text style={RecordScreenStyle.colHeader}>
                日期: {selectedDate}
            </Text>
    ...
    switch (refractive) {
        case "0": {
            for (const date of subArray)
                output.push(
                    isLeft ? dataArr[date].L_Myopia : dataArr[date].R_Myopia
                );
            break;
        }
        case "1": {
            for (const date of subArray)
                output.push(
                    isLeft
                        ? dataArr[date].L_Hyperopia
                        : dataArr[date].R_Hyperopia
                );
            break;
        }
        case "2": {
