...
export const VerificationInput = ({
  ...
  const selectedIndex =
    values.length < CELL_COUNT.length ? values.length : CELL_COUNT.length - 1;
        ...
        <View style={styles.content}>
          ...
          <View style={styles.wrap}>
            ...
            {CELL_COUNT.map((v, index) => {
              ...
              return (
                <View
                  style={[styles.cell, isLastCell ? { marginRight: 0 } : null]}
                  key={index}
                >
                  <Text style={styles.code}>{values[index] || ""}</Text>
                </View>
              );
            })}
          </View>
