    ...
    componentDidMount() {
        database
            ...
            .on("value", (snapshot) => {
                ...
                if (
                    JSON.stringify(this.state.uid_list) !=
                    JSON.stringify(temp_uid_list)
                ) {
                        ...
                        <View style={{ flex: 1 }}>
                            <View style={GetEducatedScreen.topArticleContainer}>
                                <TouchableOpacity onPress={pressHandler}>
                                    <Image
                                        source={{
                                            uri: this.state.topArticle.image,
                                        }}
                                        style={
                                            GetEducatedScreen.topArticleImage
                                        }
                                    />
                                    <Text
                                        style={GetEducatedScreen.topArticleText}
                                    >
                            ...
                            <View
                                style={GetEducatedScreen.articleListContainer}
                            >
                                <FlatList
    ...
    return (
        <TouchableOpacity onPress={pressHandler}>
            <View style={GetEducatedScreen.articleItem}>
                ...
                />
                <View style={{ flex: 1 }}>
                    <Text style={GetEducatedScreen.articleSubject}>
                        {item.subject}
                    </Text>
                    <Text style={GetEducatedScreen.articleDate}>
                        {item.date}
                    </Text>
